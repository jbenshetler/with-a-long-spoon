...
- [ ] **Cross-chapter fact audit (Lane A)** — the class every per-chapter
  ...
  `audits/fact-audit/<model>/`. Runs **three independent ledgers** (Anthropic,
  OpenAI, Google) and keeps only what more than one lands on. **Over-flags by
  design — the author rules on every item.** Token rule: subscription lanes are
  the default; OpenRouter is paid and author-authorized only. **In progress —
  three models over the first ten chapters (2026-09-19); dates are out of
...
- [ ] **Orphaned-reference sweep (Lane B)** — text still pointing at something
  ...
  `tools/orphan_refs.py`; sweeps in `audits/orphan-refs/`. Deterministic and
  ...
  (`audits/orphan-refs/vol1-2026-09-19.txt`) — candidates outstanding, not yet
  ruled with the author.**
