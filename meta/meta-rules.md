# Character Rules — Verification Checklist

*This file is a verification instrument, not a teaching document.*
*For the reasoning behind any rule, open the relevant craft or architecture file.*
*Run this checklist against any draft scene before considering it complete.*

---

## VEE

**Voice and register**
- [ ] Voice regresses under pressure — smaller, younger, simpler — never up into precision or sophistication
- [ ] Body responds before mind permits — response rendered first, cognition follows
- [ ] Her signature deflection is preemptive self-exposure as a joke, not armor-reaching
- [ ] The mother is audible in dialogue only as displaced prim judgment of *other* women's sexuality (*floozy* etc.) — never self-directed in dialogue
- [ ] With Cassie: unguarded-bawdy. With Randi: warmth-pressure takes her voice. With Pace: deflection fires early and recedes with trust

**Interior and shame**
- [ ] The cutting voice is rendered at experience level — it cuts her; she does not narrate running it on herself
- [ ] The most defended nerve is involuntary sexual signals (wetness, flush, hard nipples) — not chosen acts
- [ ] Shame is constitutional and kept; the arc is ownership replacing suppression, not liberation from shame

**Behavioral tells**
- [ ] Hand to collarbone when something surprises her emotionally
- [ ] Goes still and inward when processing something real; makes a joke when she has no toolkit for it
- [ ] The snide/prudish commentary about "girls like that" is present early, absent late — never announced, just gone
- [ ] The writing-off (*"sorority thing"*) is active, not passive — she notices she's doing it

**What the prose must hold**
- [ ] The goods of the year are rendered at full force, in real time — no pre-contamination with the future reveal
- [ ] Her no at the end is the equipment working, not damage or ingratitude
- [ ] The reader inhabits her — interior felt *with* her, not observed from outside
- [ ] Do not suggest she will find a healthy relationship that surpasses what she had with Pace — there is no free lunch

---

## PACE

**Interior and POV**
- [ ] Interior hotter than behavior — hot fantasist, severe discipline; the heat is real and felt at the edges of the held line
- [ ] Interior voice runs in chivalric cadences, mathematical rigor, bar-quote epistemology — no contemporary therapeutic vocabulary
- [ ] Present-moment perception at peak; the after-register does not run — render the asymmetry, not a general failure of integration
- [ ] No filing, cataloging, or registering-and-storing — he is in continuous present-tense contact, more like a musician improvising than an analyst reviewing data

**The grammar and the vow**
- [ ] He does not say *I love you*, *I am yours*, or *I promise* — the not-declaring is the grammar operating, not a failure
- [ ] The vow holds the line below consciousness — render it as habit, not decision
- [ ] Porch-scene-style discomfort has three concurrent sources (substrate, spirit-of-rules, wanting-to-say-it) — none surfaced, small body-fact only

**What the prose must not do**
- [ ] No coercion beats — both women say yes to everything, freely, and mean it
- [ ] No interim cruelty — he does not hurt Vee until the final hurt; harm is structural, not local
- [ ] Do not soften him to reduce culpability; do not exaggerate him to increase it
- [ ] Do not write scenes in which no honorable option was available — the options were available; he did not take them
- [ ] The available honorable exits (disclosure, refusal, harder questions) were real — prose must not protect him from this
- [ ] Damage produced organization, not disorder — no residual flashes where the father's voice surfaces

**The chivalric frame**
- [ ] He slides toward *not-finally-the-lady* reading after Vee leaves — pity, sincere, wrong; do not let him revise
- [ ] The forms (dance, mathematics, kink, dressmaking) are his retreats from the formless — strip them at the end
- [ ] Excellent at 95%, not perfect — render small lapses without signaling them as failures or the goodness as fraudulent

---

## RANDI

**Speech and instrument**
- [ ] Power wears warmth — bright, fast, affirming, endearment-laden; she does not chill, withhold, or belittle
- [ ] Control lives *inside* the warmth, never instead of it — dominance moves arrive wrapped in affirmation, deniable
- [ ] The affirmation engine goes quiet with Pace — render the contrast without explaining it
- [ ] She does not narrate her own appetite, restriction, or wound

**The reserved vocabulary**
- [ ] *Seen* / *known* / *perceived* / *met* / *beheld* belong to Pace — never use them for what Randi produces in Vee
- [ ] Randi makes Vee feel *recognized* / *chosen* / *cool* / *clever* / *admitted* — conferred status, not being-known

**The cup**
- [ ] Cup/nail-press fires on bottom-position (being-done-to) — when something is done to her she cannot control
- [ ] Cup does NOT fire when Randi is operating/dominant — the operation itself is the discharge
- [ ] Never name the gesture as a symbol
- [ ] Cup debuts at the bench — that scene stamps it as the bottom's gesture; subsequent appearances inherit the valence

**The locked door**
- [ ] Randi locks or bolts every door she enters — first act, every time, plain behavior, never stated as symbol

**The SATC scenes**
- [ ] Sadism is present from the start, not discovered — she is already operating at adult resolution
- [ ] The genuine like is real — she actually enjoys the lunches, actually likes Vee; sadism and love coexist
- [ ] Render the concealment from Pace — let the reader see what she omits; let the reader see his happy reception of the sanitized report

**What the prose must hold**
- [ ] She is tragic, not fated — the wound presented its channel and she chose it, half-seeing
- [ ] The hate survives the novel intact — do not let her spend it or articulate it to anyone
- [ ] The sadism and the genuine love are the same motion — do not flatten her into pure operator or sentimentalize her into victim
- [ ] POV is always outside Randi — assemble her interior from surface tells; never render her interiority directly

---

## ALL SCENES — Cross-Cutting Rules

**The structural argument**
- [ ] The chiasmus (Pace/Randi parallel refusals; Vee/Randi parallel-opposite transformations) never surfaces in dialogue or narration — the reader feels it from outside
- [ ] No character is cast as villain, manipulator, or naïf
- [ ] The configuration is the agent of harm — not any single party
- [ ] No character advances who has not half-seen — render the half-seeing without making it full-seeing

**The seen/recognized distinction**
- [ ] *Seen* belongs to Pace — reserved exclusively for the real-interior-perceived-and-met register
- [ ] Never use seeing-register vocabulary (*known, perceived, met, beheld*) for what Randi produces in Vee

**POV consistency**
- [ ] We are never inside Randi's POV — she is assembled from behavior and tells
- [ ] The reader inhabits Vee — interior felt with her, not observed
- [ ] Pace observed from outside except in designated Pace-POV scenes

**Goods and harm**
- [ ] The goods of the configuration are real in real time — never pre-contaminated
- [ ] The harm is structural and cumulative, not local — no single scene is the catastrophe
- [ ] Both women's transformations are real — Vee's integrates, Randi's does not; neither outcome is a verdict on either woman

---

## BEFORE FILING A DRAFT

- [ ] Chronology entry updated — summary reflects what is actually on the page, not the original plan (`meta-plan-chronology.md`)
- [ ] Status updated to *draft complete* / *partial* / etc.
- [ ] File reference added or confirmed in the chronology entry
- [ ] Any new canonical facts (dates, objects, character details, established phrasings) noted in the relevant arch or bible doc
- [ ] Any continuity issues created or resolved flagged in the chronology's continuity flags section

*For the reasoning behind any rule: `meta-craft-vivienne.md`, `meta-craft-pace.md`, `meta-craft-randi.md`, `meta-arch-bible.md`*

---

## WRITING STYLE — Synthesized from the Bench and the Pointing Game

*These rules describe how the prose operates in the drafted exemplar scenes. Use them to calibrate new drafts and to verify that a completed scene is written in the novel's register.*

**Sentence rhythm and pacing**
- [ ] Sentences earn their length — short sentences land events; long sentences track continuous action or interior observation. Rhythm is not decorative.
- [ ] The prose slows down for the things that matter. Duration on the page = weight in the scene. Do not rush through the load-bearing beats.
- [ ] Repetition is intentional and structural — the same word or phrase returning does work (*she locked it behind her, first thing / the script was failing / the bench had been waiting*). Accidental repetition reads as error; purposeful repetition reads as insistence.
- [ ] Dialogue is short. The short line earns silence; silence earns the next line. Do not fill silence with dialogue.
- [ ] No false antithesis / empty balance — "not X but Y," "X, not Y," "X didn't make it less Y," "less X than Y" earn the shape only when X and Y genuinely exclude each other and the exclusion is the point. Delete-test: cut the negated half; if only cadence is lost, cut it for real. Enact a real contradiction in behavior or image; never discharge it into a balanced clause *about* the contradiction.

**Interiority and observation**
- [ ] POV character reads through behavior first, then infers interior — never the reverse. He *felt* the small recoil in her hand; he *heard* the lock turn. The inference follows the sensation.
- [ ] The POV character notices specifics, not categories. Not *she was nervous* — the specific physical tell. Not *he was attracted* — where his attention goes and how long it stays.
- [ ] Interior heat is rendered at the edges of held behavior, not stated directly. The reader feels what the character is controlling by watching the control exert itself.
- [ ] When something matters to the POV character, the prose stays with it. The redhead gets paragraphs; the others get sentences. Duration tracks significance.

**Objects and environment**
- [ ] Objects carry weight when they have been established. The bench, the scrunchies, the shirt, the bowl of thin-sliced apple — introduced without emphasis, returned to with accumulation. Plant without flagging; return without explaining.
- [ ] Environment is specific and purposeful. The sugar maple gone orange in a field of things that had not yet decided to change. Not mood-setting — *meaning* carried in concrete image. If the environment isn't doing work, it isn't there.
- [ ] What a character doesn't say about an object tells you about the character. She looked at the wood. She did not look at it the way she looked at her own things. The inference is the reader's.

**What the prose withholds**
- [ ] The prose does not editorialize. It does not explain what the scene means or tell the reader how to feel. The bench is described; its function is described; the reader assembles the implication.
- [ ] Characters do not narrate their own psychology. They act from it. *She reached past him to the bowl* — not *she decided to seem casual* or *she wanted to feel in control.*
- [ ] Irony is structural, not verbal. The most devastating moments — Pace's contented plans, Randi's smile settling back on — are rendered in the prose's flat register, without the narrative winking.

**The body**
- [ ] Physical response precedes cognition and is rendered before the character processes it. She did not address it. She was not going to address it. The body first; the management second.
- [ ] Involuntary responses are rendered with precision, not euphemism. *The slick of her already gleaming between her thighs.* The prose does not look away from the body; it does not look at it provocatively either. Clinical accuracy with emotional weight.
- [ ] Touch is rendered from the inside of the touching, not as visual observation. The warmth under his palm. The small bumps of the vertebrae. The scene is felt, not filmed.

---

## SCENE DETAILS — Synthesized from Vee Tells Cassie About the Fitting

*These rules govern canonical details that must remain consistent once established. Useful for verification against existing scenes and for maintaining continuity in new ones.*

**Detail handling**
- [ ] Details established in a scene become canonical — check against `meta-plan-chronology.md` and the relevant arch/bible docs before writing anything that could conflict
- [ ] The no-tag shirt: she went looking for the tag the first night she had it; it was the softest thing she owned; good fabric, well-made; she filed it as a riddle and gave up. The fitting detonates it.
- [ ] The shirt on the pillow: she put it there without thinking when she changed — it belongs on her pillow, not Cassie's, not put away
- [ ] The collar: he made it four times to get it right. He said it like it was nothing.
- [ ] The riser: she stood on it. It put her up. He was on his knees for the hem and looking up — the geometry is specific and load-bearing
- [ ] The silk: burgundy, not russet (russet is the magazine shade she described; he went deeper and warmer, matched her coloring, not the reference)
- [ ] The heat: he turned it up before she came. Days ago, probably. Whenever he knew she was coming. He thought of it then. This is the detail that detonates Cassie — *"Oh, Vee."*
- [ ] The bra question: he stated it as a fact about the work (*"the bra changes the line"*) and then waited, left it open, left her a door. He did not use the word *ask.* But he asked.

**Cassie's register**
- [ ] Cassie puts the highlighter down the moment Vee walks in — she reads Vee instantly
- [ ] Cassie's warmth changes key: the laughter and the serious are the same attention, different expression. She knows when to let the pivot happen.
- [ ] Cassie does not editorialize about Pace. She asks questions. She waits. *"Oh, Vee"* is the single moment of unguarded response.
- [ ] The gay cross-examination: Cassie's running question (*"are you sure he's not gay?"*) is Cassie's deflection from the emotional weight — comic relief she earns by having just held the serious.
- [ ] Humor is bone-dry; she does NOT laugh at her own jokes. Tell = a slightly cocked eyebrow she WITHHOLDS for a beat, to see if you recognize the line before she confirms it. With Vee it resolves near-instantly. Reserve the failure-state (eyebrow never comes, joke silently retracted) for a Randi beat.
- [ ] She laughs readily at OTHERS, especially Vee — amusement never aimed. The un-instrumented opposite of Randi's warmth: Cassie withholds her signal until you've earned it; Randi gives hers up front to capture.
- [ ] Render her almost entirely as effect-on-Vee / she-was-right. Do not develop standalone interiority. Curiosity about Cassie for her own sake = over-feed; cut it.

**Cassie's look (anti-glamour; never scored, never staged as contrast every time)**
- [ ] Short, boxy, "unmanaged" — not cute, not a makeover-frump, not a bitter plain friend. She isn't running the femininity game. Body held off fat by sport, not vanity.
- [ ] Brown hair in a short shag (grown past a cut); no makeup; shorts in warm weather, pants in cold; no skirts. Brown is deliberate — does not twin Randi, does not flatter.
- [ ] The un-coded third model vs. Vee (lush/natural) and Randi (sleek/polished). Narrator NEVER verdicts it as more authentic. Contrast to Randi lives in managed-vs-unmanaged and is true without being staged — do not write it every time they share a page.
- [ ] Plainness is the (unstated) source of her clear sight: no skin in the appearance economy, so she reads Randi for operation instead of being caught in the beam.
- [ ] Sports: softball (spring) and basketball (winter); any rec team given the chance. Gives her a real calendar — the benign absence that aches later (the friend who'd have kept Vee safe was at a game).
- [ ] The car: old rust-spotted SUV, squeaking suspension, kept because it works (hauls equipment). Anti-status vehicle; the squeak is a free entrance cue Vee recognizes. Keep it an object — no name, no quirks, no running bit.

**Vee's telling register (Cassie channel)**
- [ ] Warmer and less performed than the Randi version will be. The architecture of the visit leaks through in the actual words — not the SATC-register shape of them.
- [ ] She moves when she's processing: to the window, back, onto the bed, up again. Still in motion at the start; settling as the telling deepens.
- [ ] The things she almost doesn't say: the heat detail, *"It was like his eyes asked, may I look."* She reaches these by talking past where she'd planned to stop.
- [ ] *"Like I was supposed to be there"* — the deepest thing she says. Arrived at after long enough silence that it's not about finding the word. It's about deciding to say it.
