...
| 7 | `DOCUMENTS.md` inventory accuracy | **RESOLVED 2026-07-20 — RETIRED (deleted).** Redundant with meta-brief + the self-describing naming convention + na.py/lore-keeper + chronology.html; a stale "master map of all files" actively misleads the LLM (false-exhaustive). Orientation folded into `meta-brief.md` as a drift-proof convention note; back-pointers cleaned (meta-brief, meta-process-chronology). |
...
**Still open (minor / author to apply):** ~~the spring-satc-bridge **chronology reorder**~~ (**APPLIED 2026-08-03** — Agreement moved to ~mid July, after First Taste) · `arch-sheri` title-marking (`{{The Usual}}`/`{{Another Round}}`) + Sheri's uncorroborated age (~20) · `cassie:46` scene-map lag (add the `{{All Told}}`/`{{Turned Up}}` Cassandra pairing) · `threesome-reveal:11` betrayal framing (no-action per audit) · deferred prose passes (`see-you-later.md` Chi Latte cover; `meta-craft-randi.md:187` Fairytale render vs the drafted NYE page).
...
- **[low/med]** `meta-arch-sheri.md` (whole doc) — never references `hills-and-valleys.md`, Sheri's first drafted on-page appearance. **Correct:** `meta-plan-chronology.md:139-140` "[VIGNETTE] Hills and Valleys · Draft complete · `hills-and-valleys.md`". **Proposed fix:** flag — add a one-line pointer to `hills-and-valleys.md`.
...
- **[PLAN] tags that are now drafted [PAGE]:** **[med/high]** `:116,119` Office; `:121,124` Woodshop; **[med/high]** `:98,112` Vee's top-drawer + mirror-ritual (drafted in `above-him.md`); **[low/high]** `:70` KitchenAid. **Fix:** retag `[PLAN]`→`[PAGE]`, cite `two-towels.md`/`above-him.md` (spring gaslight grains stay `[PLAN]`).
...
_Currency + light prose cross-check vs the drafted scene. Wave 1 (12): all-the-time, barely-stings, burn, fairytale, fed, grace, in-her-place, in-his-hands, leave-no-trace, my-friend-randi, old-acquaintances, outlier. Wave 2 (12): practice-room, recognized-method, rock, see-you-later, hills-and-valleys, still-life, the-bench, the-usual, turned-up, vee-on-the-bench, water-wings, we-find-out._
...
### meta-note-hills-and-valleys.md — 3 (med 2, low 1)
...
#### Currency
- **[low/med]** `meta-arch-cassie.md:46` — Function list gives only "Statistics project (`outlier.md`)" for the Cassandra beat; scene-map lags the drafted corpus (`all-told.md`, `turned-up.md` are now drafted Cassie-Cassandra beats). **Correct:** `meta-plan-chronology.md:121` "Pairs with {{Turned Up}} (…Cassie's Cassandra role)." **Proposed fix:** flag — if Function/tells meant to be current, add the All Told/Turned Up pairing.
