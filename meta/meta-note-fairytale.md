...
- **Location.** Fairytale → **Gstaad**, established **in-scene** (Randi's POV at the resort). On Vee's side the location reaches her only as Randi's **sparse break texts** (*"two lines and a chalet,"* `my-friend-randi.md:95`) — **no separate pre-break setup is needed.** (The old *"Gesso mid-November"* pointer was wrong twice over: `{{Gesso}}` is a **spring** SATC scene following `{{Still Life}}` (~late Jan, *after* NYE), and no off-grid beat exists.)
