...
- **Location.** This commits Fairytale → Gstaad — which **needs a pre-break setup** (Randi tells Vee she'll be off-grid over New Year's) so the midnight text can be the only contact. **That setup is currently UNWRITTEN** and no off-grid/Gstaad mention exists in the corpus yet; it must land in a **fall SATC beat before Christmas break** — **not `{{Gesso}}`** (~late Jan, *after* NYE — it can't set up a Dec 31 trip; the old "Gesso mid-November" note is doubly stale on date and content). Plant it.
