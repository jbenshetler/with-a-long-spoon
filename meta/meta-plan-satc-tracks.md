...
**The clothing-loan device (locked; see `meta-note-spring-satc-bridge.md`).** Randi *loans* Vee clothes — never gives — extending the shoe-loan motif (first rung at the {{CW-Dance Debrief}} → … → the threesome pumps). Randi is short and slender; Vee is tall and lush, so items modest on Randi run borderline-scandalous on Vee: Vee arrives at lunch displaying more than she'd have chosen, with the loan as built-in alibi (*she didn't choose it*). It's a surface-authoring rung that strips the independence wall while the erotic alarm points elsewhere — a fresh shame (public display), not a relit one.
