...
**Cold-read panel + token rule (author ruling 2026-09-05; supersedes 2026-08-19 roster).** Cold reads are **grounded only** — the chained lane is retired (it forgets too badly); its reviews are archived under `reviews/cold-read/<model-id>/chained/`. **The roster for BOTH blind instruments lives in one place: `reviews/cold-read/ensemble-config.toml`** — `[panel]` for the cold-read panel (with `fast` and `retired`), `[capture]` for the capture DAG. Print it with `tools/cold_read_config.py`. **Never restate model lists in this file or in a skill doc** (author ruling 2026-09-26: the lists drifted and a session ran the wrong model on their word); tools read the config, and `capture_dag.py` refuses a model that is not in it. Lane is derived from the id: `claude-*` runs through the headless clean lane on Claude subscription OAuth; `gpt-*` through codex subscription auth; any id with a `provider_model` in its reader block is OpenRouter — **paid, and never launched without specific author authorization** (`--auth api-key` is likewise an author-authorized escape hatch only). Donor-memory readers (`checkpoint_source = "ensemble:core"`) consume the quote-backed, cross-vendor `core` ensemble checkpoint rather than minting native checkpoints; the harness rejects native minting and records the ensemble hash in review headers. Run the panel with **`/wals-cold-read <slug>`** (contract in `reviews/cold-read/SPEC.md`). All blind lanes use a throwaway non-repo cwd or equivalent clean prompt isolation so project instructions, git state, and the memory index cannot contaminate a read. The packet-MCP subagent lane is retired for panel reads; `--persist-output` remains as salvage. Each read's memory is the resolved checkpoint `ck-ch<B>` + the raw prose window — no carry-forward chain. Durable decisions live here or in versioned review configuration/specification, never machine-local memory.
