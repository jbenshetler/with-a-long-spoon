...
**Cold-read panel + token rule (author ruling 2026-08-19; supersedes 2026-08-11).** Cold reads are **grounded only** — the chained lane is retired (it forgets too badly); its reviews are archived under `reviews/cold-read/<model-id>/chained/`. The **full panel** = `claude-fable-5` (fable), `claude-opus-4-8` (opus), `claude-sonnet-5` (sonnet), `gpt-5.6-terra` (terra), `gpt-5.6-sol` (sol), `gpt-5.5` — **all six, written to disk, run by default**. A **fast** probe = `terra` + `sonnet`. Run it with **`/wals-cold-read <slug>`** (the grounded runbook; contract in `reviews/cold-read/SPEC.md`). **Never use API tokens (`--auth api-key`) without specific author authorization**: the OpenAI trio runs through **codex** subscription auth (`tools/cold_read_grounded.py`), and the Claude trio runs through the **headless clean lane** (author ruling 2026-08-22): `tools/cold_read_grounded.py --model claude-<id>` spawns `claude -p` on **subscription OAuth** (the harness scrubs `ANTHROPIC_API_KEY`) with the agent def as the entire system prompt, from a non-repo cwd — because in-session subagents inherit ambient context (this file, the git commit snapshot, the memory index; probe-confirmed 2026-08-22) that contaminates a blind read. **This subscription behavior is the default (`--auth subscription`); `--auth api-key` (added 2026-09-02) is the author-authorized escape hatch named above** — it flips both lanes to pay-per-token billing (`OPENAI_API_KEY` for the GPT lane via the Responses API, replacing codex; `ANTHROPIC_API_KEY` left in place for the Claude `claude -p` lane), with the blind-read guarantees (throwaway non-repo cwd, excluded dynamic system-prompt sections) identical on both. The packet-MCP subagent lane (`--emit-packet` + `blind-reader-grounded`) is retired for panel reads; `--persist-output` remains as salvage. Each read's memory is the decade checkpoint `ck-ch<B>` + the raw prose window — no carry-forward chain. Durable decisions live here or in `meta/` — **not** in machine-local memory, which does not travel across clones.
