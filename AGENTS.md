...
**Scene review:** `/wals-scene-review <slug>` runs a full craft/architecture/continuity review of a drafted scene — it grounds the story-so-far via `tools/checkpoint_context.py` (decade checkpoint + raw recent prose, fed not fetched), fans out the lore-keeper prep in parallel, runs the style linter, and reports against a fixed rubric (rubric defined in that command file). Chapter-level focus. Its novel-level sibling is **`/wals-place-in-novel <slug>`** — judges the chapter's *place in the book* (arc position, characterization trajectories, architecture/thesis service, echo economy, neighbor pacing), fed the thesis/arch docs and whole-volume grounded memory up front. Both flag and advise; never rewrite prose.

**Cold-read panel + token rule (author ruling 2026-09-05; supersedes 2026-08-19 roster).** Cold reads are **grounded only** — the chained lane is retired (it forgets too badly); its reviews are archived under `reviews/cold-read/<model-id>/chained/`. The authoritative roster is `reviews/cold-read/ensemble-config.toml`. The **full panel** = `claude-fable-5`, `claude-opus-4-8`, `gpt-5.6-sol`, `gpt-5.5`, `kimi-k3`, `glm-5.3-flash`, `qwen3.8-max-0902`, and `deepseek-v4-pro-0813` — **all eight, written to disk, run by default**. A **fast** probe = `claude-opus-4-8` + `gpt-5.6-sol`. `claude-sonnet-5` and `gpt-5.6-terra` are retired from voting; Terra may serve as the non-voting checkpoint-ensemble matcher. Run the panel with **`/wals-cold-read <slug>`** (contract in `reviews/cold-read/SPEC.md`). Qwen and DeepSeek are donor-memory readers: they consume the quote-backed, cross-vendor `core` ensemble checkpoint rather than minting native checkpoints; the harness rejects native minting and records the ensemble hash in review headers. **Never use paid API tokens without specific author authorization**: Kimi, GLM, Qwen, and DeepSeek use OpenRouter only after that authorization. Fable and Opus run through the headless clean lane on Claude subscription OAuth; Sol and GPT-5.5 run through codex subscription auth. The harness default for subscription-capable models remains `--auth subscription`; `--auth api-key` is an author-authorized escape hatch only. All blind lanes use a throwaway non-repo cwd or equivalent clean prompt isolation so project instructions, git state, and the memory index cannot contaminate a read. The packet-MCP subagent lane is retired for panel reads; `--persist-output` remains as salvage. Each read's memory is the resolved checkpoint `ck-ch<B>` + the raw prose window — no carry-forward chain. Durable decisions live here or in versioned review configuration/specification, never machine-local memory.
...
Go to ground truth — a **pinpoint** lookup inline via `na.py`, a **sift/synthesis** lookup via the `lore-keeper` subagent (see **Research / lore delegation**).
...
When you need anything from the corpus, **delegate the search to the `lore-keeper` subagent** rather than reading `meta/` or `scenes/` into the main context. *The corpus* — and the `na.py` index — spans both the `scenes/` prose **and** the `meta/` planning files about the novel (thesis, architecture, chronology, track docs, TODO/open-questions, continuity flags). So "look it up" covers planning and orientation lookups, not just in-world facts. It runs in its own window: it queries the `na.py` recall-first index (falling back to `rg`), reads the candidate passages there, **filters** to the ones that actually answer, and returns those **verbatim with `file:line` sources** — keeping this session's context clean. The lore-keeper is a *retrieval instrument, not an interpreter*: it returns the relevant passages with enough fidelity to preserve nuance (it does **not** crush the answer to a single sentence, **not** dump whole files, and **not** tell you what they mean). **You** synthesize, summarize, and read the irony/hidden-thread meaning from what it returns — that judgment is yours, with the whole book in view.

**Pinpoint vs. sift — when to run it inline vs. delegate.** Now that `na.py search` returns line-anchored snippets (`lines`, both lanes), a *pinpoint* lookup — a single fact, a known passage, an exact-phrase or cross-reference check — may be run **inline** via `na.py search "…" --json` (or `--regex` for exact forms): the snippet + `file:line` is a few hundred tokens, so quoting it keeps this context clean *without* a subagent round-trip. This still honors the Prime Rule — you're going to ground truth, not answering from memory. **Delegate to the `lore-keeper` subagent** when the answer needs *sifting many candidates, reading across several files, or verifying a batch* — the work that would otherwise pull whole chapters into this context. When in doubt, or when a search returns a lot to weigh, delegate.
...
3a. **Lint the fresh draft before the author ever sees it** — `tools/novel-assistant/na.py style <path>`. Required, not "a natural moment": a rule recorded only in `style/style-rules.toml` is **invisible while drafting**, so the linter is the only thing that enforces it. Act by severity per **Style checking**, with one addition — a hit that is also a **craft rule in the Bible's Global Craft Rules** (the punctuation and reserved-word rules; `flat-interrogative` is the worked case) is a rule breach, so fix it now like an `error` rather than surfacing it as a taste call. (Four dropped question marks shipped in a chapter drafted 2026-09-12 because this step was optional and the rule lived only in lint config and a triage doc.)
3b. **If the edit CUT or REPLACED anything, hunt the orphans** — `tools/orphan_refs.py --slugs <slug> [<slug-that-recalls-it> …]`. **Pass every chapter that recalls this one,** not just the one you edited: a recalled beat lives in two files (`between.md` ↔ `barely-stings.md` is the worked pair), and the orphan surfaces in the *other* file. Don't rely on the pre-commit hook for this — it runs `--staged`, which fires after you stage, is silenced, and sees only staged paths.
3c. **Before a chapter is called done, run the cross-chapter fact audit** — `tools/fact_audit.py --slugs <slug>` (or `/wals-fact-audit`). Per revision pass, not per keystroke. It **over-flags by design and the author rules on every item** — flag, never fix. **Token rule applies:** subscription lanes are the default; **OpenRouter is paid and needs the author's explicit authorization** every time.
   - Both are *per-edit* uses of two book-level passes. **The passes themselves — what they catch, their state directories, and their status — are tracked in `meta/meta-plan-editorial-checklist.md`** (Lane A / Lane B), which is the authority on required editorial passes. Record pass progress there, never here.
...
- **Never trade the prose for a clean report (author ruling 2026-09-14).** Only `error` is ever fixed without showing the author; `warn` and `info` are surfaced and *wait*. Most rules police **overuse**, and an overused phrase is frequently still the right one in the sentence it's in — `a-small` alone carries ~217 hits, and "the small click of the lock turning over" is precise, not lazy. **A hit is not a defect, and a quiet report is not the goal.** If the honest replacement for a flagged phrase is worse prose — vaguer, more generic, a synonym reached for only to dodge a pattern — leave the line alone and tell the author the hit is there and why it should stand. Never invent a weaker image to clear a warning. And when an over-repetition *is* real, **cut instances rather than paraphrase them**: the fix for a phrase used five times is usually to use it twice, not to find five different ways to say it.
- **Within-chapter repetition is a separate instrument (`tools/echo_harvest.py`).** The linter has a *closed vocabulary* — it only finds tics someone already wrote a rule for, so it is blind to a distinctive phrase a draft happens to over-repeat. `echo_harvest.py --scene <slug>` is the open-vocabulary half: n-grams scored `local / (1 + uses elsewhere)`, so distinctive-and-concentrated rises and ambient texture drops out, with the smallest word-gap between uses reported (a tight gap is a cluster the ear catches; a wide one may be a deliberate bookend). Run it after revising a chapter. Same discipline as the linter: **flags, never findings** — the book runs on designed repetition, and separating a seeded echo from an accident is the author's ruling, not the tool's.
- **Question marks: the assistant's bias is known and one-directional (author, 2026-09-20).** It drops needed marks and has never once over-marked. The test is the author's: *if a person saying it would voice it as a question, it gets a question mark* — dialogue, italic interiority, recalled speech alike; flat only where the page marks it as not really asked, and recalled Cassie stays flat. When drafting or revising, **when in doubt, mark it**; and treat a `flat-interrogative` hit as a rule breach (fix), not a taste call. The linter cannot see an elliptical question with no interrogative word — read for those by eye.
- **`never-name` (severity `error`) is not a tic — it's a canon breach.** What the rule protects, and why, lives in `meta-orientation.md` / the Bible — deliberately not restated here (this file is inherited by the blind cold-readers). Treat any `error` hit as a real violation to flag, not a style nicety.
...
- `tools/` — book-specific tooling that operates on this repo. `chronology_html.py` generates a local, gitignored, self-contained `chronology.html` (status + beat-density view, per-scene **review pills**, embedded cold reads, and a click-through fullscreen reader for drafted scenes) from `meta/meta-plan-chronology.md`; run `tools/chronology_html.py` to regenerate. See the **review-tracking** convention below for the `reviewed:` field the pills read. Two **revision instruments** back the Lane A / Lane B passes in `meta-plan-editorial-checklist.md` and are per-edit steps above (3b, 3c): `orphan_refs.py` finds text still pointing at something an edit removed (deterministic, free), and `fact_audit.py` finds a fact one chapter contradicts in another (model-backed; subscription lanes free, OpenRouter paid and author-authorized only). Their state lives under `audits/`.
...
- `meta/meta-plan-satc-tracks.md` — authoritative on the relationship track it covers and that track's own DOs/DON'Ts (which relationship, and the details, live in `meta-orientation.md` / this doc in full — kept out of this inherited file).
...
- **New chapters: mark a late-named POV upfront.** If a new chapter keeps its POV character unnamed for a stretch (the prose runs on bare "she/her" before the name lands), add an italic POV marker at the top of the scene doc naming the POV character — and, where useful, who is *not* present — the way `scenes/the-bench.md` does. Fresh-context reader agents (`blind-reader`, `lore-keeper`, cold-read scouts) otherwise infer POV from cues and can misattribute it (happened once with `famished.md`). Volume 1 is drafting-complete, so this is for **new** chapters, not a retrofit of existing ones.
...
- **Regenerate the chronology HTML after editing the chronology.** Whenever you change `meta/meta-plan-chronology.md`, run `tools/chronology_html.py` to rebuild the local, gitignored `chronology.html` — never commit or push a chronology edit without refreshing the local view you used to review it. The page includes embedded current-draft cold reads under the scene cards. **The tracked `.githooks/pre-commit` hook automates this** (in clones that ran `git config core.hooksPath .githooks`): when a `scenes/*.md`, `meta/meta-plan-chronology.md`, or `reviews/cold-read/*.md` file is staged it regenerates `chronology.html` locally without staging generated HTML. The manual step above is the fallback for clones without the hook enabled. Note `chronology.html` embeds each scene's last-commit date (read from git), so it is **not deterministic** — it changes as prose commits land even when the chronology text does not.
...
- **Title/slug scope lock.** Display titles, slugs, `{{Title}}` references, and title provenance are immutable unless they are explicitly named as deliverables. A display-title change requires the author to say “rename/change the display title from X to Y.” Kebab-case text such as `my-pleasure` is a slug and never authorizes title searches, heading changes, reference migration, or title-paragraph replacement. Once an explicit display-title rename is authorized, migrate its `{{Title}}` references per the chapter-reference convention. Corrections remain inside the named deliverable.
- **Graded-write execution cap.** Mechanical meta edits receive one targeted diff and applicable lint, not a producer/grader loop. Large intertwined planning writes may use one producer pass and one parallel grading round. A failed substantive-content grade may trigger one revision and one regrade; once the content passes, anchor wording and other mechanical corrections receive a direct diff and lint only. Any further full grading round requires the author’s approval.
- **Stop means stop.** If the author says to stop a search, migration, or delegated task, cancel all in-flight work in that scope immediately. Do not finish the current round, continue validation, or preserve momentum at the expense of the stop instruction.
