...
**The reviews lane is opt-in.** `na.py reindex` also indexes the `reviews/` corpus (reader reactions, at `reviews/<category>/<model>/[<volN>/]<scene-slug>.md` — e.g. the live lane `reviews/grounded-cold-read/claude-opus-4-8/vol1/the-bench.md`), but only the `## Reader reaction` section of each file — and it is **excluded from default `search`**, so reviewer opinion never contaminates canon recall. Reach it *only on explicit request* with `na.py search "<q>" --reviews` (add `--category grounded-cold-read` / `--model claude-opus-4-8` / `--volume vol1` to scope; any of these implies `--reviews`). The `reviews/_archive/` (retired chained cold read) and `reviews/_harness/` (shared config/state) lanes are excluded from the index entirely. Reviews are *reactions to* the novel, **not canon** — don't treat a review's reading as a fact about the book, and the `lore-keeper` should not consult them unless a task explicitly asks about the reviews themselves.
...
- `reviews/` — reader reactions to drafted scenes, organized by **instrument** (category), then model, then volume: `reviews/<category>/<model>/[<volN>/]<scene-slug>.md`. Categories: **`grounded-cold-read/`** — the live reader instrument (volume-split, e.g. `reviews/grounded-cold-read/claude-opus-4-8/vol1/the-bench.md`); `judge/` — spec-blind fidelity judge (volume-split); `oracle/` — end-of-volume oracle battery (flat, not human-read); `_archive/cold-read/` — the **retired** chained cold read (superseded because its summary-of-a-summary carry-forward lost load-bearing facts; see `meta/meta-note-bounded-reader.md`); `_harness/` — shared config/state (SPEC, README, ORACLE, `oracle-battery.json`, `volume-packets.toml`, `principals.toml`, `.packets/`). Grounded reads carry a `## Reader reaction` section (no carry-forward). **Reactions, not canon.** `na.py` indexes only the `## Reader reaction` section, keeps it out of default search (reach it with `search --reviews`), and excludes `_archive/`+`_harness/` entirely. `tools/reading_order.py <slug>` shows the reading-order window around a scene.
- `checkpoints/` — spec-blind decade **memory checkpoints** (`checkpoints/<model>/ck-chNNN.md`): the grounded reader's faithful memory of earlier chapters, minted by `tools/checkpoint_extract.py`. **Not reviews** — a shared asset consumed by both the grounded reader and drafting/revision (`tools/checkpoint_context.py`), which is why it sits at top level rather than under `reviews/`.
