...
GROUNDED = REPO / "reviews" / "cold-read"


def grounded_exists(model: str, slug: str) -> bool:
    return (GROUNDED / model / f"{slug}.md").is_file()
...
def main() -> None:
    ...
    for s in window:
        ...
        if args.model:
            tag = "✓" if grounded_exists(args.model, s["slug"]) else "✗"
        else:
