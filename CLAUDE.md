...
> `Document authority` bullets below domain-only, without the reveal specifics.
...
**Scene review:** `/wals-scene-review <slug>` runs a full craft/architecture/continuity review of a drafted scene — it fans out the lore-keeper prep in parallel, runs the style linter, and reports against a fixed rubric (craft rules, character architecture, sensory grounding, thesis-carry, seed-calibration, earn-the-dark, continuity/dates). Flags and advises; never rewrites prose. Defined in `.claude/commands/wals-scene-review.md`.
...
**Verify scene participants before attributing anything to Vee.** A recurring error: several Pace-POV scenes render the woman as an unnamed "she" for long stretches, and she is often not Vee. Before crediting any contact, first, or trait to Vee, confirm the scene's POV/participants from its header and chronology entry — and when delegating a Vee continuity check, don't pre-label candidate scenes as Vee scenes (that primes the error); tell the lore-keeper to verify participants itself. (The specific recurring confusions — which scenes, which woman — are listed in `meta-orientation.md`.)
...
- `meta/meta-thesis.md` — the structural argument (the "why" under everything). See `meta-orientation.md` for the summary; this doc is authoritative in full.
...
- The **climax scene's companion note** (its `meta-note-*.md`; the chronology's entry names it) — authoritative on that scene's structure and its ending. (Deliberately not named here — `CLAUDE.md` is inherited by the `blind-reader`, and the filename is a tell.)
