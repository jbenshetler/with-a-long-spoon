...
5. When a prior ledger is supplied, copy its SHA-256 into `prior_ledger_sha256`. Reconcile by stable `slot`: restate a still-current temporal claim, replace it with a newly supported claim in the same slot, or omit it to leave it carried forward as unresolved. Never treat omission as supersession. An immutable claim in an existing slot cannot be changed.
