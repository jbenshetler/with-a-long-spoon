...
**Fallback — grep + read.** If `na.py` errors, has no index, or returns nothing useful, use the OMP `grep` tool over `meta/` + `scenes/` and read the hits directly. Same job, slower. For exact-form lookup, try `na.py search --regex` first — it survives an Ollama outage and keeps the provenance tags.
